...
const reinforcementLearning = require('./reinforcementLearning');
...
class Dashboard {
  ...
  async initialize() {
    ...
    try {
      ...
      // Start server - explicitly use port 3002 to avoid conflicts
      this.server = this.app.listen(3002, () => {
        logger.info(`Dashboard running on port 3002`);
      });
    } catch (error) {
  ...
  setupRoutes() {
    ...
    this.app.get('/api/reinforcement/stats', async (req, res) => {
      try {
        const rlStats = reinforcementLearning.getStatistics();
        res.json(rlStats);
      } catch (error) {
        logger.error(`Error in reinforcement learning stats API: ${error.message}`);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
    
    this.app.get('/api/reinforcement/missed', async (req, res) => {
      try {
        const limit = parseInt(req.query.limit || '10');
        // Get the most recent missed opportunities
        const missedOpportunities = reinforcementLearning.missedOpportunities
          .sort((a, b) => b.timestamp - a.timestamp)
          .slice(0, limit);
        res.json(missedOpportunities);
      } catch (error) {
        logger.error(`Error in missed opportunities API: ${error.message}`);
        res.status(500).json({ error: 'Internal server error' });
      }
    });
        ...
        <html>
          ...
          <body>
            ...
            <div class="card">
              ...
              <div id="reinforcement-stats" class="stats">Loading...</div>
              <h3>Recent Missed Opportunities</h3>
              <div id="missed-opportunities">Loading...</div>
            </div>
            ...
            <script>
              ...
              async function fetchData() {
                try {
                  ...
                  // Fetch reinforcement learning stats
                  const rlStatsRes = await fetch('/api/reinforcement/stats');
                  const rlStatsData = await rlStatsRes.json();
                  
                  document.getElementById('reinforcement-stats').innerHTML = 
                    '<div class="stat-item">' +
                    '  <h3>Total Rejected Tokens</h3>' +
                    '  <p>' + rlStatsData.totalRejectedTokens + '</p>' +
                    '</div>' +
                    '<div class="stat-item">' +
                    '  <h3>Missed Opportunities</h3>' +
                    '  <p>' + rlStatsData.totalMissedOpportunities + '</p>' +
                    '</div>' +
                    '<div class="stat-item">' +
                    '  <h3>Missed Rate</h3>' +
                    '  <p>' + (rlStatsData.missedOpportunityRate * 100).toFixed(2) + '%</p>' +
                    '</div>';
                  
                  // Fetch missed opportunities
                  const missedRes = await fetch('/api/reinforcement/missed');
                  const missedData = await missedRes.json();
                  
                  let missedHtml = '<table><tr><th>Token</th><th>Performance</th><th>Timeframe</th><th>Rejection Reason</th></tr>';
                  
                  if (missedData.length === 0) {
                    missedHtml += '<tr><td colspan="4">No missed opportunities</td></tr>';
                  } else {
                    missedData.forEach(missed => {
                      missedHtml += 
                        '<tr>' +
                        '  <td>' + missed.tokenSymbol + '</td>' +
                        '  <td class="profit">' + (missed.performance * 100).toFixed(2) + '%</td>' +
                        '  <td>' + missed.timeframe + '</td>' +
                        '  <td>' + missed.reasons.join(', ') + '</td>' +
                        '</tr>';
                    });
                  }
                  
                  missedHtml += '</table>';
                  document.getElementById('missed-opportunities').innerHTML = missedHtml;
